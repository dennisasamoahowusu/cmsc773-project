...
def do_topic_modelling(corpus, output_dir, task, model=None):
...
for task in range(len(df_ir['task'].unique())):
    ...
    lda_model = make_lda(corpus, id2word)
    tfidf_model = make_lda(corpus_tfidf, id2word)
    ...
    do_topic_modelling(corpus, "./outputs", task, lda_model)
    do_topic_modelling(corpus_tfidf, "./tfidf_outputs", task, tfidf_model)
